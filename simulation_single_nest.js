...
class AntForagingSimulation {
    ...
    initialize() {
        ...
        // Place single nest in center
        const nestMargin = 100;
        this.nest.x = this.width / 2 + (Math.random() - 0.5) * 200; // Center with some variation
        this.nest.y = this.height / 2 + (Math.random() - 0.5) * 200;
        
        // Keep nest within bounds
        this.nest.x = Math.max(nestMargin, Math.min(this.width - nestMargin, this.nest.x));
        this.nest.y = Math.max(nestMargin, Math.min(this.height - nestMargin, this.nest.y));
        ...
        this.baseDistance = Math.min(this.width, this.height) * 0.3; // 30% of smaller canvas dimension
        this.placementAngles = [];
        
        // Pre-calculate angles for all 4 pairs (distribute around the nest)
        for (let pair = 0; pair < 4; pair++) {
            const angle = (pair * Math.PI / 2) + (Math.random() - 0.5) * 0.5; // 90° apart with variation
            this.placementAngles.push({ angle });
        }
    ...
    drawApprovalScreen() {
        ...
        if (this.approvalStep === 0) {
            this.ctx.fillText('STEP 0: Single nest placed in center.', this.width / 2, panelY + 60);
            this.ctx.fillText('One colony will forage for food.', this.width / 2, panelY + 80);
            ...
            this.ctx.fillText('1 red apple + 1 gray obstacle blocking path.', this.width / 2, panelY + 80);
            this.ctx.fillText('Approve or Reject?', this.width / 2, panelY + 100);
        } else if (this.approvalStep === 5) {
            this.ctx.fillText('Complete! All 4 apple + obstacle pairs placed.', this.width / 2, panelY + 60);
            this.ctx.fillText('4 food sources and 4 obstacles total.', this.width / 2, panelY + 80);
    ...
    placeNextAppleObstaclePair() {
        ...
        // Place food (red apple)
        const food = new Food(0, 0);
        food.pos.x = this.nest.x + Math.cos(angles.angle) * distance;
        food.pos.y = this.nest.y + Math.sin(angles.angle) * distance;
        
        // Validate food position
        let foodValid = this.isValidFoodPosition(food);
        
        if (foodValid) {
            this.foodSources.push(food);
            
            // Immediately place obstacle for this food
            this.placeObstacleBetweenPoints(food.pos, new Vec(this.nest.x, this.nest.y));
        }
    }
