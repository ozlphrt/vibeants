...
class AntForagingSimulation {
    ...
    draw() {
        ...
        // Draw pheromone trails (WebGL or CPU)
        this.drawPheromones();
    ...
    drawPheromones() {
        if (this.useWebGL && this.webglRenderer) {
            ...
            const homeColor = { r: 64, g: 64, b: 64 }; // Dark grey for scouting
            const minColor = hexToRgb(this.minSignal || '#8b6b47');
            const maxColor = hexToRgb(this.maxSignal || '#ff8c00');
            const foodColor = { r: maxColor.r, g: maxColor.g, b: maxColor.b };
        ...
        for (let x = 0; x < this.pheromoneField.gridW; x += stepSize) {
            for (let y = 0; y < this.pheromoneField.gridH; y += stepSize) {
                ...
                if (foodStr > 2) {
                    ...
                    const smoothedAlpha = this.performanceMode ? alpha : this.applyGaussianSmoothing(x, y, 'food', alpha);
...
function saveDefaultColors(simulation) {
    try {
        const colors = {
            ...
            minSignal: simulation.minSignal,
            maxSignal: simulation.maxSignal
        };
