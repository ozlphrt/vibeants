...
class AntForagingSimulation {
    constructor(canvas) {
        ...
        this.placeObstaclesAlongPath = (path, count) => {
            for (let i = 0; i < count; i++) {
                ...
                // Don't place too close to nests (increased distance)
                ...
                // Don't place obstacles too close to existing obstacles
    ...
    initialize() {
        ...
        for (let i = 0; i < this.foodCount; i++) {
            ...
            do {
                ...
                let ok = true;
                // Don't place food too close to either nest (increased distance)
                ...
                // Don't place food too close to other food (increased distance)
        ...
        // Create obstacles strategically to block line of sight between food and nests
        this.obstacles = [];
        
        // First, ensure each food source has obstacles blocking direct path to both nests
        for (let food of this.foodSources) {
            // Calculate direct paths from food to both nests
            const pathToNest1 = this.getDirectPath(food.pos, new Vec(this.nest.x, this.nest.y));
            const pathToNest2 = this.getDirectPath(food.pos, new Vec(this.nest2.x, this.nest2.y));
            
            // Place obstacles along these paths
            this.placeObstaclesAlongPath(pathToNest1, 1);
            this.placeObstaclesAlongPath(pathToNest2, 1);
        }
        
        // Add additional random obstacles to fill the space
        ...
        for (let i = 0; i < remainingObstacles; i++) {
            ...
            do {
                ...
                let ok = true;
                // Don't place obstacles too close to either nest (increased distance)
                if (obstacle.pos.subtract(new Vec(this.nest.x, this.nest.y)).magnitude() < obstacle.baseRadius + this.nest.radius + 100 ||
                    obstacle.pos.subtract(new Vec(this.nest2.x, this.nest2.y)).magnitude() < obstacle.baseRadius + this.nest2.radius + 100) {
                    ok = false;
                }
                // Don't place obstacles too close to food sources (increased distance)
                for (const food of this.foodSources) {
                    if (obstacle.pos.subtract(food.pos).magnitude() < obstacle.baseRadius + food.radius + 50) {
                        ok = false;
                        break;
                ...
                // Don't place obstacles too close to each other (increased distance)
                for (const existingObstacle of this.obstacles) {
                    if (obstacle.pos.subtract(existingObstacle.pos).magnitude() < obstacle.baseRadius + existingObstacle.baseRadius + 30) {
                        ok = false;
                        break;
