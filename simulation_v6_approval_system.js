...
class AntForagingSimulation {
    ...
    initialize() {
        ...
        // Create strategic food and obstacle placement (4 pairs, 8 food sources, 8 obstacles)
        this.foodSources = [];
        this.obstacles = [];
        
        // Calculate base distance from nests (proportional to canvas size) - TESTED AND APPROVED
        const baseDistance = Math.min(this.width, this.height) * 0.4; // 40% of smaller canvas dimension
        
        for (let pair = 0; pair < 4; pair++) {
            // Calculate distance for this pair (slight variation for natural look)
            const distance = baseDistance + (Math.random() - 0.5) * 50; // ±25px variation
            
            // Calculate angles for food placement (distribute around the center)
            ...
            food1.pos.x = this.nest.x + Math.cos(angle1) * distance;
            food1.pos.y = this.nest.y + Math.sin(angle1) * distance;
            
            // Place food for colony 2
            const food2 = new Food(0, 0);
            food2.pos.x = this.nest2.x + Math.cos(angle2) * distance;
            food2.pos.y = this.nest2.y + Math.sin(angle2) * distance;
            
            // Validate food positions (ensure they're within bounds and don't overlap)
        ...
        // If we didn't get enough food sources, add some additional ones
        while (this.foodSources.length < this.foodCount) {
            let food;
            let attempts = 0;
            do {
                ...
                if (this.isValidFoodPosition(food)) break;
            } while (attempts < 100);
        ...
        this.isRunning = false;
        console.log('Placement ready for approval - waiting for user input');
    }
    ...
    drawApprovalScreen() {
        ...
        this.ctx.fillText('Strategic Placement Preview', this.width / 2, panelY + 40);
        
        // Draw description
        this.ctx.fillStyle = 'white';
        this.ctx.font = '16px Arial, sans-serif';
        this.ctx.fillText('Review the placement of nests, food sources, and obstacles.', this.width / 2, panelY + 80);
        this.ctx.fillText('Each colony gets 4 food sources at equal distances.', this.width / 2, panelY + 105);
        this.ctx.fillText('Obstacles block direct paths between food and nests.', this.width / 2, panelY + 130);
    ...
    canvas.addEventListener('click', (e) => {
        ...
        if (simulation.needsApproval) {
            ...
            if (simulation.approveButton && 
                ...
                console.log('Approval granted - simulation started');
                return;
            }
            ...
            if (simulation.rejectButton && 
                ...
                console.log('Approval rejected - regenerating placement');
                return;
            }
        }
