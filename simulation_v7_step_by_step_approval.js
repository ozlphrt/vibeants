...
class AntForagingSimulation {
    constructor(canvas) {
        ...
        this.isValidObstaclePosition = (obstacle) => {
            ...
            for (const food of this.foodSources) {
                if (obstacle.pos.subtract(food.pos).magnitude() < obstacle.baseRadius + food.radius + 50) {
                    return false;
                }
            }
    ...
    initialize() {
        ...
        this.isRunning = false;
        this.approvalStep = 1; // Track current step: 1=nests, 2=food, 3=obstacles, 4=complete
        this.currentPair = 0; // Track current food/obstacle pair
    ...
    drawApprovalScreen() {
        ...
        this.ctx.fillText('Strategic Placement - Step by Step', this.width / 2, panelY + 40);
        
        // Draw step info
        this.ctx.fillStyle = 'white';
        this.ctx.font = '16px Arial, sans-serif';
        
        if (this.approvalStep === 1) {
            this.ctx.fillText('Step 1: Nests have been placed on opposite sides.', this.width / 2, panelY + 80);
            this.ctx.fillText('Orange nest (Colony 1) and Green nest (Colony 2).', this.width / 2, panelY + 105);
            this.ctx.fillText('Click "Next Step" to place the first pair of food sources.', this.width / 2, panelY + 130);
        } else if (this.approvalStep === 2) {
            this.ctx.fillText(`Step 2: Food Pair ${this.currentPair + 1}/4`, this.width / 2, panelY + 80);
            this.ctx.fillText('Red apple (for Colony 1) and Green apple (for Colony 2).', this.width / 2, panelY + 105);
            this.ctx.fillText('Check the distances. Click "Approve" if fair, or "Reject" for feedback.', this.width / 2, panelY + 130);
        } else if (this.approvalStep === 3) {
            this.ctx.fillText(`Step 3: Obstacles for Pair ${this.currentPair + 1}/4`, this.width / 2, panelY + 80);
            this.ctx.fillText('Gray obstacles placed between each food and its nest.', this.width / 2, panelY + 105);
            this.ctx.fillText('Click "Approve" if placement looks good, or "Reject" for feedback.', this.width / 2, panelY + 130);
        } else if (this.approvalStep === 4) {
            this.ctx.fillText('Complete! All 4 pairs have been placed.', this.width / 2, panelY + 80);
            this.ctx.fillText('8 food sources and 8 obstacles total.', this.width / 2, panelY + 105);
            this.ctx.fillText('Perfectly balanced competition between colonies.', this.width / 2, panelY + 130);
        }
        
        // Draw stats
        this.ctx.fillStyle = '#ffa500';
        this.ctx.font = 'bold 18px Arial, sans-serif';
        this.ctx.fillText(`Food Sources: ${this.foodSources.length}`, this.width / 2, panelY + 170);
        this.ctx.fillText(`Obstacles: ${this.obstacles.length}`, this.width / 2, panelY + 195);
        ...
        if (this.approvalStep === 1) {
            ...
            this.ctx.fillText('NEXT STEP', nextX + buttonWidth / 2, buttonY + 26);
            ...
            this.ctx.fillStyle = 'white';
            this.ctx.font = 'bold 16px Arial, sans-serif';
            this.ctx.fillText('START SIMULATION', startX + buttonWidth / 2, buttonY + 26);
            ...
            this.ctx.fillStyle = 'white';
            this.ctx.font = 'bold 16px Arial, sans-serif';
            this.ctx.fillText('APPROVE', approveX + buttonWidth / 2, buttonY + 26);
            ...
            this.ctx.fillStyle = 'white';
            this.ctx.font = 'bold 16px Arial, sans-serif';
            this.ctx.fillText('REJECT', rejectX + buttonWidth / 2, buttonY + 26);
    ...
    placeObstaclesForCurrentPair() {
        if (this.currentPair >= 4) return;
        
        // Get the last two food sources (current pair)
        const food1 = this.foodSources[this.foodSources.length - 2];
        const food2 = this.foodSources[this.foodSources.length - 1];
        
        if (food1 && food2) {
            // Place obstacles between food and nests
    ...
    nextApprovalStep() {
        if (this.approvalStep === 1) {
            // Step 1: Place first food pair
            this.placeNextFoodPair();
            this.approvalStep = 2;
        } else if (this.approvalStep === 2) {
            // Step 2: Place obstacles for current pair
            this.placeObstaclesForCurrentPair();
            this.approvalStep = 3;
        } else if (this.approvalStep === 3) {
